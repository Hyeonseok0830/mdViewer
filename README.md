...
- **백링크 & 아웃고잉 링크** — 이 노트를 참조하는/이 노트가 참조하는 파일 목록을 사이드바에 표시
...
- **파일 탐색기** — 트리 뷰, 들여쓰기 가이드, 즐겨찾기(★), 최근 파일, 드래그&드롭 이동,
  파일/폴더 컨텍스트 메뉴 (생성·이름변경·삭제), 빈 폴더 표시
- **링크 안전 이름 변경** — 노트 이름을 바꾸면 볼트 전체의 위키링크 자동 재작성
- **템플릿** — `templates/` 폴더 + `{{date}}` `{{time}}` `{{title}}` 변수
- **슬래시 명령** — 에디터에서 `/` 입력으로 제목·표·콜아웃 등 즉시 삽입
- **이미지 붙여넣기** — 클립보드 이미지를 `attachments/`에 자동 저장 후 링크
